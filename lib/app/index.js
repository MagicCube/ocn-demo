...
console.info('\nWechat');
console.info(`AppID: ${config.get('wechat.appId')}`);
console.info(`AppSecret: ${config.get('wechat.appSecret')}\n`);
...
function updateAccessToken() {
  request({
    ...
    console.info('[ATU]');
    setTimeout(updateAccessToken, 30 * 60 * 1000);
  }, (err) => {
    console.error(err);
    setTimeout(updateAccessToken, 30 * 60 * 1000);
  });
...
app.use((req, res, next) => {
  if (
    !req.session.wechat &&
    (
      config.get('security.allowAnonymous').indexOf(req.path) === -1) &&
      !req.path.startsWith('/assets/') &&
      !req.path.startsWith('/api/movie/')
    )
  {
